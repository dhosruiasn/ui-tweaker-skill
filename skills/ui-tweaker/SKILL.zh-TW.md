...
控制面板底部「確認，更新程式碼」按鈕使用 sendPrompt() 將所有當下數值以結構化格式送回 Claude。
...
Claude 收到 `[UI Tweaker 確認]`（或其英文形式 `[UI Tweaker Confirm]`，預設樣板送出的就是英文版）開頭的訊息後：
1. 解析所有數值
...
- 收到 [UI Tweaker 確認] 訊息後，直接輸出程式碼，不再詢問
