...
- **Computed color parsing must preserve alpha**: color, shadow, and format-copy helpers must parse both legacy comma RGB (`rgba(0,0,0,.12)`) and modern space/slash RGB (`rgb(0 0 0 / .12)` or percentages). Never fall back to opaque black when parsing a valid modern color string; that makes the first shadow or copied style adjustment visually explode.
