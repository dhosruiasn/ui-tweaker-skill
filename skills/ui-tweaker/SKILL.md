...
- **Right-click context menu**: right-clicking the preview should open a compact dark context menu for the target under the cursor. Required actions: `Copy values`, `Paste here`, `Paste to same kind`, `Select deepest layer`, and `Select next outer layer`. Copied values are style/control state only, not text content. `Paste to same kind` applies the copied state to equivalent repeated items (same semantic suffix such as `flowerBack` → `mushroomBack` / `hiddenBack` / `seedlingBack`, or otherwise same layer label), so users can tune one repeated component and push its values to matching siblings without adjusting each by hand. The custom menu must also intercept right-clicks on selection overlays (`#tw-tbox`, selected-frame overlays, handles), not only the real preview DOM; otherwise a selected element can show the browser's native menu instead of the panel menu.
