...
- **Mandatory self-check after generating (three steps, do them before opening the panel)**:
  ...
  4. On a real browser, spot-check 2–3 elements via `getComputedStyle` (size / `flex` / color) and confirm they match the real component's rendering.
...
⑧ Color
   ...
   - **SVG icon exception**: when the selected element is an SVG (e.g. a line-style pin icon), the Color category should expose **fill `fill` / stroke `stroke`** (separately, each with a "none (`none`)" toggle for transparent paint) plus **`stroke-width`**, instead of background/text/border — an SVG's tunable color surface is `fill`/`stroke`, not background/border. Read initial values via `getComputedStyle` as usual (`fill:none` → default the "none" toggle on and disable the swatch; `stroke` reads back the resolved currentColor rgb). Apply via `el.style.setProperty('fill'|'stroke', val, 'important')`; emit `fill:` / `stroke:` in the confirm output. Detect with `el.tagName.toLowerCase()==='svg'`.
...
When the user wants to edit the "display text" itself (system copy, button labels, placeholders, titles…) and not just the styling, the control panel can offer a "Text content" input at the **top** of the "① Typography" category for text-bearing elements:

- **Read/write the right target per element type**: plain elements edit `textContent`; `<input>` edits `placeholder` (the hint shown to the user; edit `value` only if that's what's displayed); `<select>` edits the first `<option>`'s text. Keep a small map noting which mode each editable key uses.
...
- **Each slot can be hidden/shown independently (eye toggle)**: add an eye toggle next to each text row, storing `_phHidden`/`_valHidden` flags; when hidden, `applyInputText` renders that slot as an empty string but **keeps the original value in state/ORIG_TEXT**, so toggling it back restores it.
...
- When useful, offer a "Text content" input at the top of "① Typography" so the user can edit the display copy directly; emit it as `text-content:` on confirm and note that the real string source must be edited.
