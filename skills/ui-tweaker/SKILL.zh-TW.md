...
- **帶拉寬手把的三欄版面**：外殼使用 `grid-template-columns: var(--tw-left-w, 230px) 6px 1fr 360px`。左＝Layers/Pages 樹、6px 軌道＝可拖曳拉寬手把、中＝預覽舞台、右＝控制面板。左欄必須能垂直捲動且可拉寬，拉寬後的寬度要用 `localStorage` 記住，深層圖層名稱才可讀。Layer row 必須替右側控制（如鎖頭／眼睛 toggle）保留固定空間；左欄過窄時，圖層名稱用 `text-overflow:ellipsis` 顯示 `...`，不可把右側控制擠出可視範圍。
