...
- **Photoshop/Figma-style canvas pan**: the preview pane must support holding `Space` and dragging to pan the scrollable preview canvas, especially while zoomed. Ignore the shortcut while typing in inputs/selects/textareas/contenteditable elements, keep toolbar clicks usable, temporarily disable preview hit-testing while panning, and call the overlay update functions during pan so selection/hover frames stay aligned. Spacebar panning takes priority over transform/selection overlays: while `.space-pan` is active, `#tw-tbox`, `#tw-selected`, hover overlays, and parent overlays should use `pointer-events:none`, so dragging over a selected element still pans the canvas instead of moving or reselecting it.
